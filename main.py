...
class Info:
    ...
    def title():
        ...
        while True: os.system(f'title L3m0nTokenGen - Generated: {Info.genned} ^| Solved Captchas: {Info.bypassed} ^| l3m0ngen.wtf')
...
class Menu:
    ...
    Info.cls()
    print('  _                             _____     _               ____            ')
    print(' | |    ___ _ __ ___   ___  _ _|_   _|__ | | _____ _ __  / ___| ___ _ __  ')
    print(" | |   / _ \ '_ ` _ \ / _ \| '_ \| |/ _ \| |/ / _ \ '_ \| |  _ / _ \ '_ \ ")
    print(' | |__|  __/ | | | | | (_) | | | | | (_) |   <  __/ | | | |_| |  __/ | | |')
    print(' |_____\___|_| |_| |_|\___/|_| |_|_|\___/|_|\_\___|_| |_|\____|\___|_| |_|')
    proxytype = 'http'
    Info.write(f"{color}[!]{Fore.RESET} What Usernames do you want in the tokens?{color}:{Fore.RESET} ")
    username = input()
    Info.write(f"{color}[!]{Fore.RESET} What Server do you want to spam (remove the {color}discord.gg/{Fore.RESET} bit from your invite)?{color}:{Fore.RESET} discord.gg/")
    invite = input()
    Info.write(f'{color}[!]{Fore.RESET} How Many Tokens{color}?{Fore.RESET} ')
    maxThreads = input()
    if not maxThreads.isdigit():
        Info.write(f'{color}[!]{Fore.RESET} Type a number you dumba-')
                ...
                with Client(transport=SyncProxyTransport.from_url(f'{Menu.proxytype}://{self.GetProxy()}')) as request:
                    r = request.post('https://discord.com/api/v9/auth/register', headers=self.headers,
                                     json={'username': f'{Menu.username} | {secrets.token_urlsafe(4)}',
                                           'email': secrets.token_hex(6) + '@gmail.com', 'password': 'sUb2lernOnC40',
